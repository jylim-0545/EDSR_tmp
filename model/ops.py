...
class Upsampler(nn.Sequential):
    def __init__(self, scale, nFeat, bn=False, act=False, bias=True):
        ...
        if (scale & (scale - 1)) == 0:    # Is scale = 2^n?
            ...
            #modules.append(conv(nFeat, 9 * nFeat, 3, bias))
            modules.append(nn.Conv2d(in_channels=nFeat, out_channels=25 * nFeat, kernel_size=3, stride=1, padding=1, bias=bias))
            modules.append(nn.PixelShuffle(5))
            if bn: modules.append(nn.BatchNorm2d(nFeat))
            if act: modules.append(act())
...
class Downsampler(nn.Sequential):
    def __init__(self, scale, nFeat, outFeat, bn=False, act=False, bias=True):
        super(Downsampler, self).__init__()

        modules = []
        modules.append(nn.Conv2d(in_channels=nFeat, out_channels=int(outFeat/(scale**2)), kernel_size=3, stride=1, padding=1, bias=bias))
        modules.append(nn.PixelShuffle(scale))
        if bn: modules.append(nn.BatchNorm2d(nFeat))
        if act: modules.append(act())
        self.downsampler = nn.Sequential(*modules)
